test_that("get_colname_by_type", {
  tst_json <- '{"source": {"type": "type_1"}, "release_scheduled": {"type": "type_2"}, "entityId": {"type": "type_3"}}'

  expected_output <- c("source", "entityId")
...
test_that("get_renamed_colnames", {
  tst_json <- '{"source": {"display_name": "SOURCE"}, "release_scheduled": {"display_name": "RELEASE DATE"}, "entityId": {"display_name": "SYNAPSE ID"}}'

  expected_output <- c("SOURCE", "RELEASE DATE", "SYNAPSE ID")
...
test_that("get_na_replace_colnames", {
  tst_json <- '{"source": {"na_replace": "Unknown"}, "release_scheduled": {"type": "type_1"}, "entityId": {"na_replace": "NO ID"}}'
...
test_that("get_na_replace_defs", {
  tst_json <- '{"source": {"na_replace": "Unknown"}, "release_scheduled": {"type": "type_1"}}'
  tst_manifest <- data.frame(source = c("Center A", NA, "Center B"),
