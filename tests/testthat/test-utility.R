...
test_that("convert_column_type", {
  ...
  expected_output <- data.frame(int = as.integer(1), factor = as.factor("dog"), logic = TRUE, char = "1", date = as.Date("2020-01-01"))
...
test_that("true_false_icon", {
  ...
  expected_output <- c(as.character(icon("check", lib = "font-awesome")), as.character(icon("times", lib = "font-awesome")))
